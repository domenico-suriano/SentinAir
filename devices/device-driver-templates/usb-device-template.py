...
CONNECTION_TYPE = "usb" # mandatory string which indicates that the device is interfaced by an USB socket. Do not modify this one.
...
DEVICE_BAUD_RATE = your_device_baud_rate # mandatory integer value indicating the usb communication baud rate
""" for example:
...
class Usb-device-template: #it is mandatory that the class name must have the same name of the file (usb-device-template.py) with the capital first letter
...
## the mandatory function "connect" check if your device is plugged into one of the SentinAir Usb port.
...
## where the function is going to search your device
    ...
    def connect(self,portname):
        ...
        try:
            ...
            """
            ###############################
            self.portname = portname ## if your device is found at the passed usb port, the device usb port name is set.
            found = 1
