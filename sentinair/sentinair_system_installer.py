...
def show_commands():
    ...
    print ("5) to list all tha avaialble drivers, type \"l\" and the \"Enter\" key")
    print ("6) to get help, press \'h\' and the \"Enter\" key")
    print ("7) to quit, press \'q\' and the \"Enter\" key\n")
...
def list_drivers(drv_dir):
    try:
        lf = os.listdir(dirmanager + DEVICES_DIR)
        for f in lf:
            if f.find("__init__") >= 0:
                continue
            if f.find(".py") >=0:
                print(f)
    except:
        print("Error in listing the available drivers")
...
while 1:
    ...
    if cmd == 'h':
        show_commands()
        continue
    if cmd == 'l':##list all the available drivres
        list_drivers(dirmanager)
        continue
